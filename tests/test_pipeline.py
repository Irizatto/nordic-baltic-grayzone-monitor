...
class PipelineTests(unittest.TestCase):
    def test_source_error_reuses_previous_real_records_and_sar(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            scorer = lambda item, *_args: {**item,"risk_score":0,"risk_level":"Normal","triggered_rules":[],"nearest_infrastructure":{"name":"x","type":"cable","distance_km":20}}
            with patch.object(pipeline,"DOCS_DATA",docs), patch.object(pipeline,"USE_MOCK_DATA",False), patch.object(pipeline,"fetch_digitraffic_ais",side_effect=requests.RequestException("offline")), patch.object(pipeline,"fetch_barentswatch_ais",return_value=None), patch.object(pipeline,"fetch_gfw_sar",return_value=([{"detection_id":"new-mock","lat":0,"lon":0,"timestamp":"2026-07-11T00:00:00Z","matched":False,"source":"mock"}],"error_kept_old_data","offline")), patch.object(pipeline,"update_history",return_value={}), patch.object(pipeline,"score_vessel",side_effect=scorer), patch.object(pipeline,"record_events"), patch.object(pipeline,"write_digitraffic"), patch.object(pipeline,"write_barentswatch"), patch.object(pipeline,"write_gfw_sar"):
                result = pipeline.run()
